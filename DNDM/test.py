...
parser.add_argument('--resume', '-r', default='llcm_agw_p4_n6_lr_0.1_seed_0_best.t', type=str,
...
def extract_gall_feat(gall_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, input1, label) in enumerate(gall_loader):
            ...
            feat_pool, feat_fc = net(input, input1, input, input1, test_mode[0])
...
def extract_query_feat(query_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, input1, label) in enumerate(query_loader):
            ...
            input = Variable(input.cuda())
            input1 = Variable(input1.cuda())
            feat_pool, feat_fc = net(input, input1, input, input1, test_mode[1])
...
if dataset == 'dn348':
    ...
    # testing set
    query_img, query_label = process_test_dn348(data_path, modal='night')
    gall_img, gall_label = process_test_dn348(data_path, modal='day')
    ...
    gallset = TestDataOld1(data_path + 'day/', gall_img, gall_label, transform=transform_test,
                           img_size=(args.img_w, args.img_h))
    queryset = TestDataOld1(data_path + 'night/', query_img, query_label, transform=transform_test,
                            img_size=(args.img_w, args.img_h))
...
if dataset == 'dnwild':
    ...
    # testing set
    query_img, query_label = process_test_dnwild(data_path, modal='night')
    gall_img, gall_label = process_test_dnwild(data_path, modal='day')
